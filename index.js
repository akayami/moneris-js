...
//--
const cleanse = function(str,spaces){
	if(spaces){
...
module.exports = function(configuration){
	...
	const firstElement = function(arr,assertion){
		return Array.isArray(arr) && arr.length>0 && arr[0] ? (assertion ? arr[0]===assertion: arr[0]) : null;
	};
	const normalizeExpiry = function(format,expiry){
	...
	const pay = function(args){
		...
		const expdate = normalizeExpiry(configuration.expiryFormat,cleanse(args.expiry,true));
		...
		const order_id = args.order_id || cleanse(configuration.app_name,true)+'-Purchase-'+suffix;
